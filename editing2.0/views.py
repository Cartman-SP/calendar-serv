    ...
    if request.method == 'GET':
        ...
        if not profile.first_usluga:
            if serializer.is_valid():
                ...
                profile.save()
                return Response(True, status=201, renderer_classes=[JSONRenderer])
        if serializer.is_valid():
            serializer.save()
            return Response(False, status=201, renderer_classes=[JSONRenderer])
        return Response(serializer.errors, status=400, renderer_classes=[JSONRenderer])
